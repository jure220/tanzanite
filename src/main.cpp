...
const string strMessageMagic = "Tanzanite Signed Message:\n";
...
{
    string strBlockFn = strprintf("tzt%04u.dat", nFile);
...
bool LoadExternalBlockFile(FILE* fileIn)
{
    ...
    {
        try {
            CAutoFile tztdat(fileIn, SER_DISK, CLIENT_VERSION);
            unsigned int nPos = 0;
            while (nPos != (unsigned int)-1 && tztdat.good())
            {
                boost::this_thread::interruption_point();
                unsigned char pchData[65536];
                do {
                    fseek(tztdat, nPos, SEEK_SET);
                    int nRead = fread(pchData, 1, sizeof(pchData), tztdat);
                    if (nRead <= 8)
                    {
                        nPos = (unsigned int)-1;
                        break;
...
{
    RenameThread("tanzanite-loadtzt");
